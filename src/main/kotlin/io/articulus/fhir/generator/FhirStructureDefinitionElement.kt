...
class FhirStructureDefinitionElement(val profile: FhirStructureDefinition,
                                     element: JsonObject,
                                     val isMainProfileElement: Boolean = false) {
    val log by logger()
    ...
    init {
        ...
        max =
                if (element.has("max"))
                    if (element["max"].asString.contains("*")) -1 else element["max"].asInt
                else 0
